...
public class MinimumFallingPathSum {
    ...
    class Solution {
        public int minFallingPathSum(int[][] matrix) {
            ...
            for (int i = 1; i < m; i++) {
                dp[i][0] = Math.min(dp[i - 1][0], dp[i - 1][1]) + matrix[i][0]; // 最后一列第一个位置
                for (int j = 1; j < n - 1; j++) {   // 最后一列中间位置
                    dp[i][j] = Math.min(Math.min(dp[i - 1][j - 1], dp[i - 1][j]), dp[i - 1][j + 1]) + matrix[i][j];
                }
                dp[i][n - 1] = Math.min(dp[i - 1][n - 2], dp[i - 1][n - 1]) + matrix[i][n - 1]; // 最后一列最后一个位置
            }
            for (int i = 0; i < n; i++) {   // 取最后一列的最小值
                if (dp[m - 1][i] < minSum)
                    minSum = dp[m - 1][i];
            }
            return minSum;
